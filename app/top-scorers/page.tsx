...
const positionGradients: Record<string, string> = {
  FW: "from-green-500/20 to-green-500/5",
  MF: "from-blue-500/20 to-blue-500/5",
  DF: "from-amber-500/20 to-amber-500/5",
  GK: "from-purple-500/20 to-purple-500/5",
};
...
export default function TopScorersPage() {
  const allScorers = useMemo(() => getTopScorers(200), []);
  const [page, setPage] = useState(0);
  const totalPages = Math.ceil(allScorers.length / PER_PAGE);
  const scorers = allScorers.slice(page * PER_PAGE, (page + 1) * PER_PAGE);
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span className="tabular-nums">{allScorers.length} scorers</span>
          <span className="text-muted-foreground/40">·</span>
          <Goal className="h-3.5 w-3.5" />
          <span className="tabular-nums">{allScorers.reduce((s, x) => s + x.goals, 0)} total goals</span>
        </div>
      </div>
              ...
              {[1, 0, 2].map((pos) => {
                ...
                return (
                  ...
                  >
                    ...
                    <PlayerAvatar name={s.playerName} size="lg" className="border-2 border-border group-hover:ring-2 group-hover:ring-primary/50 transition-all" />
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative shrink-0">
                      <PlayerAvatar name={s.playerName} size="md" className="border border-border group-hover:border-primary/50 transition-colors" />
