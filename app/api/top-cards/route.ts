...
import { computeTopCardsFromResults } from "@/lib/data-service";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cards = computeTopCardsFromResults(allResults, limit);
    return NextResponse.json(cards, {
      headers: { "Cache-Control": "public, s-maxage=60, stale-while-revalidate=300" },
    });
  } catch {
    return NextResponse.json({ error: "Failed to load top cards" }, { status: 500 });
  }
}
